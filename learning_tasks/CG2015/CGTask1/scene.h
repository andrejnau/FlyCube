...
public:
	...
	virtual void draw()
	{
		...
		for (int i = 0; i < this->modelOfMemoryL.size(); ++i)
		{
			if (glm::distance(g, rotate * modelOfMemoryL[i].vertices[0]) < 1e-1)
			{
				cur_id = i;
				break;
			}
		...
		if (cur_id == this->modelOfMemoryL.size() - 1)
			full_turn = true;

		lenta_a.x += dt;
		lenta_b.x += dt;

		glm::vec3 lenta_c = rotate * modelOfMemoryL[cur_id].vertices[1];
		glm::vec3 lenta_d = rotate * modelOfMemoryL[cur_id].vertices[0];
