...
void* IMalloc::Alloc(size_t size)
{
    return malloc(size);
}
void* IMalloc::Realloc(void* ptr, size_t size)
{
    return realloc(ptr, size);
}
void IMalloc::Free(void* ptr)
{
    free(ptr);
}
...
void* CAllocator::Allocate(size_t nBytes) throw()
{
    return malloc(nBytes);
}
void CAllocator::Free(void* p) throw()
{
    free(p);
}
...
{
    uint32_t* blobOut = (uint32_t*)malloc(sizeof(uint32_t) + (ui + 1) * sizeof(OLECHAR));
