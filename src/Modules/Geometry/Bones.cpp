...
{
    if (!bones_info_srv)
        bones_info_srv = context.CreateBuffer(BindFlag::kSrv, static_cast<uint32_t>(bone_info.size() * sizeof(BoneInfo)));
    if (!bone_info.empty())
...
{
    if (!bone_srv)
        bone_srv = context.CreateBuffer(BindFlag::kSrv, static_cast<uint32_t>(bone.size() * sizeof(glm::mat4)));
    if (!bone.empty())
