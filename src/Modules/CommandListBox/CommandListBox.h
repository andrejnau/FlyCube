...
public:
    ...
    void Attach(const NamedBindKey& bind_key, const std::shared_ptr<DeferredView>& view);
    ...
    void EndEvent();
    void ClearColor(const NamedBindKey& bind_key, const std::array<float, 4>& color);
    void ClearDepth(const NamedBindKey& bind_key, float depth);
