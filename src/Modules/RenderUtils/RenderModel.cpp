...
std::shared_ptr<Resource> RenderModel::CreateTextureFromFile(const std::string& path)
{
    ...
    if (path.ends_with(".dds") || path.ends_with(".ktx") || path.ends_with(".kmg")) {
        ...
        res = m_device->CreateTexture(TextureType::k2D, BindFlag::kShaderResource | BindFlag::kCopyDest, format,
        ...
        res = m_device->CreateTexture(TextureType::k2D, BindFlag::kShaderResource | BindFlag::kCopyDest,
