...
public:
    ...
    struct CBuffer
    {
        ...
        {
        public:
            UniformProxy(CBuffer& self, const std::string& name)
                : m_self(self)
                , m_name(name)
            {
            }
            ...
            {
                auto& desc = m_self.m_vdesc[m_name];
                *reinterpret_cast<T*>(m_self.m_data.data() + desc.offset) = value;
            }
            ...
            , m_shader_type(shader_type)
        {
            ...
            for (UINT i = 0; i < bdesc.Variables; ++i)
            {
                ...
                auto& d = m_vdesc[vdesc.Name];
                d.size = vdesc.Size;
                d.offset = vdesc.StartOffset;
            }
        }
    ...
    std::map<std::string, CBuffer> vs_buffers;
    std::map<std::string, CBuffer> ps_buffers;

    CBuffer& GetVSCBuffer(const std::string& name)
    {
        auto it = vs_buffers.find(name);
        if (it != vs_buffers.end())
            return it->second;
        throw std::exception("name not found");
    }
    ...
    {
        auto it = ps_buffers.find(name);
        if (it != ps_buffers.end())
            return it->second;
        throw std::exception("name not found");
    }
    ...
    {
        vertex_shader_buffer = CompileShader(device, vertex, "main", "vs_5_0");
        ASSERT_SUCCEEDED(device->CreateVertexShader(vertex_shader_buffer->GetBufferPointer(), vertex_shader_buffer->GetBufferSize(), nullptr, &vertex_shader));

        pixel_shader_buffer = CompileShader(device, pixel, "main", "ps_5_0");
        ASSERT_SUCCEEDED(device->CreatePixelShader(pixel_shader_buffer->GetBufferPointer(), pixel_shader_buffer->GetBufferSize(), nullptr, &pixel_shader));
        ...
        ASSERT_SUCCEEDED(device->CreateInputLayout(layout, ARRAYSIZE(layout), vertex_shader_buffer->GetBufferPointer(),
            vertex_shader_buffer->GetBufferSize(), &input_layout));

        ParseVariable(device, ShaderType::kVertex, vertex_shader_buffer, vs_buffers);
        ParseVariable(device, ShaderType::kPixel, pixel_shader_buffer, ps_buffers);
    }
};
